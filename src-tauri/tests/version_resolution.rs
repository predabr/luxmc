...
async fn test_1_21_4() {
    ...
    println!("  [JAVA] major_version={} (from javaVersion field)", java_major);
    assert!(java_major >= 17, "1.21.4 should require Java 17+, got {}", java_major);

    let has_modern = detail.arguments.is_some();
    println!("  [ARGS] format = {}", if has_modern { "MODERN" } else { "OLD" });
    ...
    assert!(detail.asset_index.is_some(), "1.21.4 should have asset_index");
    assert!(detail.main_class.is_some(), "1.21.4 should have main_class");
    let mc = detail.main_class.as_deref().unwrap();
    println!("  [MAIN] {}", mc);
    assert!(mc.contains("Minecraft") || mc.contains("Main"),
        "1.21.4 main_class should be a Minecraft class, got {}", mc);

    let lib_count = detail.libraries.len();
    println!("  [LIBS] {} libraries", lib_count);
    assert!(lib_count > 50, "1.21.4 should have many libraries, got {}", lib_count);

    let mut url_ok = 0;
    for lib in &detail.libraries {
        if !launcher::is_library_allowed(lib) { continue; }
        if let Some(ref dl) = lib.downloads {
            if let Some(ref artifact) = dl.artifact {
                if !artifact.url.is_empty() { url_ok += 1; }
            }
        }
    }
    ...
    assert!(unresolved.is_empty(), "1.21.4 has unresolved vars: {:?}", unresolved);
...
async fn test_1_16_5() {
    ...
    assert!(java_major >= 8 && java_major <= 16, "1.16.5 should be Java 8-16, got {}", java_major);
    ...
    let resolved = resolve_game_args(&detail, &game_args);
    let unresolved: Vec<_> = resolved.iter().filter(|a| a.contains("${")).collect();
    assert!(unresolved.is_empty(), "1.16.5 has unresolved vars: {:?}", unresolved);
...
async fn test_1_8_9() {
    ...
    let java_major = detail.java_major_version();
    println!("  [JAVA] major_version={} (default, no javaVersion field)", java_major);
    ...
    for lib in &detail.libraries {
        ...
        if let Some(ref dl) = lib.downloads {
            ...
            if !url.is_empty() && url.starts_with("http") { url_ok += 1; }
        }
    }
    println!("  [LIBS] url_valid={}", url_ok);
    assert!(url_ok > 15);
    ...
    let resolved = resolve_game_args(&detail, &game_args);
    let unresolved: Vec<_> = resolved.iter().filter(|a| a.contains("${")).collect();
    assert!(unresolved.is_empty(), "1.8.9 has unresolved vars: {:?}", unresolved);
...
async fn test_b1_7_3() {
    ...
    let game_args = detail.effective_game_args();
    println!("  [ARGS] resolved {} game args from minecraftArguments", game_args.len());
    assert!(!game_args.is_empty(), "b1.7.3 should resolve game args from minecraftArguments string");
    ...
    assert!(main_class.contains("Minecraft") || main_class.contains("Launch"),
        "b1.7.3 main_class should be a Minecraft/Launch class, got {}", main_class);

    assert!(detail.downloads.is_some(), "b1.7.3 should have downloads in Mojang manifest");
    assert!(detail.asset_index.is_some(), "b1.7.3 should have asset_index");
    assert!(detail.assets.is_some(), "b1.7.3 should have assets field");

    let lib_count = detail.libraries.len();
    println!("  [LIBS] {} libraries", lib_count);
    assert!(lib_count > 5, "b1.7.3 should have libraries, got {}", lib_count);
    ...
    for lib in &detail.libraries {
        if !launcher::is_library_allowed(lib) { continue; }
        if lib.downloads.is_some() { with_downloads += 1; }
        if lib.url.is_some() { with_url += 1; }

        if let Some(ref dl) = lib.downloads {
            if let Some(ref artifact) = dl.artifact {
                if !artifact.url.is_empty() { url_ok += 1; } else { url_bad += 1; }
            } else {
                let url = lib_url_from_name(&lib.name, lib.url.as_deref());
                if !url.is_empty() && url.starts_with("http") { url_ok += 1; } else {
                    url_bad += 1;
                    println!("  WARN: lib {} no artifact, constructed URL: {}", lib.name, url);
                }
            }
        } else {
            let url = lib_url_from_name(&lib.name, lib.url.as_deref());
            if !url.is_empty() && url.starts_with("http") { url_ok += 1; } else {
                url_bad += 1;
                println!("  WARN: lib {} no downloads, constructed URL: {}", lib.name, url);
            }
        }
    }
    println!("  [LIBS] with_downloads={}, with_url_field={}, url_valid={}, url_invalid={}",
        with_downloads, with_url, url_ok, url_bad);
    assert_eq!(url_bad, 0, "b1.7.3 should have no invalid library URLs");

    let resolved = resolve_game_args(&detail, &game_args);
    let unresolved: Vec<_> = resolved.iter().filter(|a| a.contains("${")).collect();
    assert!(unresolved.is_empty(), "b1.7.3 has unresolved vars: {:?}", unresolved);
