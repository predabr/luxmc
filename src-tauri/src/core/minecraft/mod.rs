...
impl VersionDetail {
	...
	pub fn effective_game_args(&self) -> Vec<String> {
					...
					if let Some(val) = obj.get("value") {
						if let Some(s) = val.as_str() {
							...
							result.extend(arr.iter().filter_map(|v| {
								let s = v.as_str()?;
								if s == "--demo" { None } else { Some(s.to_string()) }
							}));
						}
					}
				}
		...
		if let Some(ref mc_args) = self.minecraft_arguments {
			return mc_args.split(' ').filter(|s| !s.is_empty()).map(String::from).collect();
		}
		vec![
			"--username".into(), "${auth_player_name}".into(),
			"--gameDir".into(), "${game_directory}".into(),
			"--assetsDir".into(), "${assets_directory}".into(),
			"--version".into(), self.id.clone(),
		]
	}
...
mod tests {
	...
	fn all_official_version_types_are_preserved() {
		...
		assert!(versions.iter().any(|v| v.id == "26.2" && v.version_type == "release"));
