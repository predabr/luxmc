...
use crate::core::minecraft::{VersionDetail, DownloadEntry, Library, lib_url_from_name};
...
impl DownloadManager {
	...
	pub async fn download_version(
		...
		Self::emit_log(self, &format!("Starting download for version {}", detail.id));
		...
		if let Some(ref downloads) = detail.downloads {
			...
			Self::emit_progress(self, &DownloadProgress {
				phase: "client".into(),
				total: 1,
			...
			if !already {
				...
				self.download_client_with_retry(downloads, &detail.id, &version_dir).await?;
			} else {
	...
	async fn download_assets(&self, asset_index: &crate::core::minecraft::AssetIndex) -> AppResult<()> {
		...
		if !index_path.exists() {
			Self::emit_log(self, &format!("Downloading asset index: {}", asset_index.id));
			Self::emit_progress(self, &DownloadProgress {
			...
			Self::emit_log(self, &format!("Asset index already exists: {}", asset_index.id));
		}
		...
		Self::emit_log(self, &format!("Downloading {} assets ({} bytes)", total, total_bytes));
		...
		for (url, size, hash) in to_download {
			...
			completed += 1;
			Self::emit_progress(&mgr, &DownloadProgress {
				phase: "assets".into(),
				total,
				completed,
				current_file: hash,
		...
		for lib in libraries {
			...
			if let Some(ref downloads) = lib.downloads {
				if let Some(ref artifact) = downloads.artifact {
					...
					to_download.push((artifact.url.clone(), path.clone(), artifact.sha1.clone(), lib.name.clone()));
					continue;
				}
			}
			...
			while join_set.len() >= MAX_CONCURRENT_DOWNLOADS {
				if let Some(result) = join_set.join_next().await {
					match result {
						...
						Ok(Err(e)) => {
							join_set.abort_all();
							return Err(AppError::Internal(format!("library download failed: {}", e)));
						}
						Err(e) => {
							join_set.abort_all();
	...
	pub async fn validate_version(&self, detail: &VersionDetail) -> AppResult<()> {
		if let Some(ref downloads) = detail.downloads {
			let client_path = self.versions_dir()
				.join(&detail.id)
				.join(format!("{}.jar", detail.id));
			if !client_path.exists() {
				return Err(AppError::NotFound(format!("client jar for {} not downloaded", detail.id)));
			}
			if !downloads.client.sha1.is_empty() {
				...
				if computed != downloads.client.sha1 {
					return Err(AppError::Internal(format!(
						"client jar sha1 mismatch for {}", detail.id
					)));
				}
			}
		}
		...
		for lib in &detail.libraries {
			...
			let path = lib_path_from_name(&lib_dir, &lib.name);
			if !path.exists() {
				return Err(AppError::NotFound(format!("library {} not downloaded", lib.name)));
			}
		}

		if let Some(ref asset_index) = detail.asset_index {
			let index_path = self.assets_dir()
				.join("indexes")
				.join(format!("{}.json", asset_index.id));
			if !index_path.exists() {
				return Err(AppError::NotFound(format!("asset index {} not downloaded", asset_index.id)));
			}
		}
...
async fn download_file_retry(
	...
	for attempt in 1..=MAX_RETRIES {
		match download_file_once(mgr, entry, path, label).await {
			Ok(()) => {
				if attempt > 1 {
					DownloadManager::emit_log(mgr, &format!("Download succeeded on attempt {} for {}", attempt, label));
				}
				return Ok(());
			}
			Err(e) => {
				DownloadManager::emit_log(mgr, &format!(
...
async fn download_file_once(
	...
	while let Some(chunk) = stream.next().await {
		let chunk = chunk.map_err(|e| AppError::Internal(format!("stream error for {}: {}", label, e)))?;
		...
		if last_report.elapsed().as_millis() >= SPEED_UPDATE_INTERVAL_MS as u128 {
			...
			DownloadManager::emit_progress(mgr, &DownloadProgress {
