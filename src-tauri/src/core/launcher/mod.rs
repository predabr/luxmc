...
impl GameLauncher {
	...
	pub async fn launch(
		...
		let game_args = self.build_game_args(detail, username, uuid, access_token, user_type, game_dir);
		...
		let safe_jvm_args: Vec<String> = jvm_args
			.iter()
			.enumerate()
			.filter_map(|(idx, a)| if rejected.contains(&idx) { None } else { Some(a.clone()) })
			.collect();
		...
		let safe_game_args: Vec<String> = game_args
			.iter()
			.enumerate()
			.filter_map(|(idx, a)| if rejected_game.contains(&idx) { None } else { Some(a.clone()) })
			.filter(|a| a != "--demo")
			.collect();
			...
			match child.wait().await {
				Ok(status) => {
					...
					if let Some(ref app) = app_exit {
						let _ = app.emit("game-exit", GameExitEvent {
	...
	async fn prepare_natives(&self, detail: &VersionDetail) -> AppResult<PathBuf> {
			...
			if lib.name.contains("natives-linux") {
				...
				if path.exists() {
					let output = Command::new("unzip")
						.args(["-o", "-j", path.to_str().unwrap_or(""), "-d", natives_dir.to_str().unwrap_or("")])
						.output()
						.await;
					match output {
						Ok(out) => {
							if !out.status.success() {
								...
								self.emit_log(&format!("unzip failed for {}: {}", lib.name, stderr));
							}
						}
						Err(e) => {
		...
		if cfg!(target_os = "linux") {
			match ensure_flite_library(&natives_dir).await {
				...
				Ok(false) => self.emit_log("Flite narrator unavailable; Minecraft will continue without narration"),
				Err(error) => self.emit_log(&format!("Flite narrator unavailable: {}", error)),
			}
		}

		let so_count = std::fs::read_dir(&natives_dir)
			.map(|rd| rd.filter(|e| e.as_ref().map(|f| f.path().extension().map(|ext| ext == "so").unwrap_or(false)).unwrap_or(false)).count())
			.unwrap_or(0);
		self.emit_log(&format!("Extracted {} native libraries to {}", so_count, natives_dir.display()));
			...
			for arg in jvm_args {
				...
				for s in values {
					let resolved = s
						...
						.replace("${library_directory}", &self.downloader.libraries_dir().to_string_lossy())
...
async fn ensure_flite_library(natives_dir: &PathBuf) -> AppResult<bool> {
	...
	let Some(source) = candidates.iter().map(PathBuf::from).find(|path| path.is_file()) else {
		return Ok(false);
	};
	...
	if bytes.len() < 4 || &bytes[..4] != b"\x7fELF" {
		return Err(crate::error::AppError::Internal(format!(
			"invalid Flite library at {}", source.display()
		)));
	}
