...
use super::{ModSearchResult, ModVersion, ModFile};

const CURSEFORGE_API: &str = "https://api.curseforge.com/v1";

pub fn api_key() -> Option<String> {
	std::env::var("CURSEFORGE_API_KEY").ok().filter(|k| !k.is_empty())
}
...
pub async fn search_mods(
	...
	let results = data
		...
		.map(|m| {
			...
			let summary = m.get("summary").and_then(|s| s.as_str()).unwrap_or("").to_string();
			let downloads = m.get("downloadCount").and_then(|d| d.as_u64()).unwrap_or(0);

			let logo = m.get("logo").and_then(|l| l.as_object());
			let icon_url = logo.and_then(|l| l.get("thumbnailUrl")).and_then(|u| u.as_str()).map(|s| s.to_string());

			let categories: Vec<String> = m.get("categories")
			...
			let versions: Vec<String> = m.get("latestFiles")
				...
				.map(|arr| {
					arr.iter()
						.filter_map(|f| f.get("gameVersion").and_then(|v| v.as_str()).map(|s| s.to_string()))
...
pub async fn get_mod_versions(
	...
	let versions: Vec<ModVersion> = data
		.iter()
		.map(|f| {
			let id = f.get("id").and_then(|i| i.as_u64()).unwrap_or(0).to_string();
			let display = f.get("displayName").and_then(|n| n.as_str()).unwrap_or("").to_string();
