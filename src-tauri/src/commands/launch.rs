...
pub async fn launch_game(
	...
	app.emit("launcher-log", format!("Starting launch for version {}", request.version_id)).ok();

	let db = crate::db::shared_db().await?;

	let account: AccountRow = sqlx::query_as::<_, AccountRow>(
		"SELECT * FROM accounts WHERE uuid = ? OR id = ?",
	)
	...
	.unwrap_or_else(|| {
		let fallback_username = if request.account_id.contains("offline") {
			request.account_id.replace("offline-", "").replace("offline_", "")
		} else {
			"Player".to_string()
		};
		AccountRow {
	...
	app.emit("launcher-log", format!("Account: {}, Version: {}, Profile: {}",
		account.username, profile.mc_version, profile.name)).ok();

	let version_row = crate::db::schema::versions::get(&db, &request.version_id)
		.await?
		.ok_or_else(|| {
			let msg = format!("Version {} not found. Try refreshing the version list.", request.version_id);
			app.emit("launcher-log", &msg).ok();
			AppError::NotFound(msg)
		})?;

	app.emit("launcher-log", format!("Fetching version detail for {}...", request.version_id)).ok();
	let detail = minecraft::fetch_version_detail(&state.http, &version_row.url).await?;
	app.emit("launcher-log", format!("Version detail fetched. Main class: {}",
		detail.main_class.as_deref().unwrap_or("unknown"))).ok();
	...
	let unzip_check = std::process::Command::new("which")
		.arg("unzip")
		.output();
	if unzip_check.is_err() || !unzip_check.unwrap().status.success() {
		app.emit("launcher-log", "WARNING: 'unzip' not found. Natives extraction may fail.").ok();
	}

	app.emit("launcher-log", "Verifying download...").ok();
	let downloader = DownloadManager::new(state.http.clone(), data_dir.clone())
		.with_app(app.clone());
	let java = JavaRuntimeManager::new(state.http.clone(), data_dir.clone())
		.with_app(app.clone());

	downloader.download_version(&detail).await?;
	app.emit("launcher-log", "Download verified. Validating version...").ok();
	downloader.validate_version(&detail).await?;
	app.emit("launcher-log", "Version validated.").ok();

	let launcher = GameLauncher::new(downloader, java)
		.with_app(app.clone());
	...
	app.emit("launcher-log", format!("Spawning Java process for {} ({}, {})...", account.username, user_type, final_uuid)).ok();
	let pid = launcher
		.launch(&detail, &account.username, &final_uuid, &final_token, user_type, &game_dir)
		.await?;

	app.emit("launcher-log", format!("Game launched with PID {}", pid)).ok();
