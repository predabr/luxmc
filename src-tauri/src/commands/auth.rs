...
pub async fn auth_login(
	...
	let listener = oauth_server::start_callback_server()
		.await
		.map_err(|e| AppError::Internal(format!("Failed to start callback server on port 8453: {}", e)))?;

	app.emit("launcher-log", "Opening browser for Microsoft login...").ok();
	open::that(&pending.url).map_err(|e| {
		let msg = format!("Failed to open browser: {}. Please open this URL manually:\n{}", e, pending.url);
		AppError::Internal(msg)
	})?;

	app.emit("launcher-log", "Waiting for login callback on http://localhost:8453/callback ...").ok();
	let callback = oauth_server::wait_for_callback(&listener, 300)
		.await
		.map_err(|e| AppError::Internal(e))?;

	app.emit("launcher-log", "Login callback received, exchanging code...").ok();
	let account = state.auth.login_with_code(&callback.code, &pending.verifier).await?;
	save_account(&state, &account).await?;
	app.emit("launcher-log", format!("Login successful: {}", account.username)).ok();
	Ok(account)
}
...
#[tauri::command]
pub async fn auth_switch_account(_state: State<'_, AppState>, uuid: String) -> AppResult<AuthAccount> {
	let db = crate::db::shared_db().await?;
...
pub async fn auth_offline_login(username: String) -> AppResult<AuthAccount> {
	...
	let uuid = Uuid::new_v5(&Uuid::NAMESPACE_URL, format!("offline:{}", username).as_bytes());
	let account = AuthAccount {
		id: uuid.to_string(),
		username,
		uuid: uuid.to_string(),
	...
	let row = AccountRow {
		...
		expires_at: Some(chrono::DateTime::from_timestamp(account.expires_at, 0)
			.unwrap_or_default()),
