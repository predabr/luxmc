	...
	for token in &tokens {
		if jvm_arg_allowed_on_current_os(token) {
			...
			if token == "-XstartOnFirstThread" {
				...
				suggestions.push(format!("Remove the flag \"{token}\" — it is not safe on this OS."));
			}
		}
	}
...
pub async fn crash_summary(lines: Vec<String>) -> AppResult<CrashSummary> {
	...
	if lower.contains("invalidcredentials") || lower.contains("401") || lower.contains("rejected by") {
		return Ok(CrashSummary {
			category: "auth".into(),
	...
	if lower.contains("outofmemory") || lower.contains("could not reserve enough space") || lower.contains("gc overhead") {
		return Ok(CrashSummary {
	...
	if lower.contains("java.lang.unsupportedclassversionerror") || lower.contains("class file version") {
		return Ok(CrashSummary {
...
mod tests {
	...
	async fn jvm_args_rejects_unsupported_flags() {
		...
		assert!(r.rejected.iter().any(|x| x.starts_with("-XX:HeapDumpPath=")));
	...
	async fn jvm_args_handles_quoted_paths() {
		let r = jvm_args_validate(r#"-Djava.library.path="/opt/mc with spaces/natives""#.to_string())
			.await
			.unwrap();
		assert!(r.valid);
		assert_eq!(r.normalized, "-Djava.library.path=/opt/mc with spaces/natives");
	}
