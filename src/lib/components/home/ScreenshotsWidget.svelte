<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			if (saved) {
				const data = z.array(z.object({ id: z.union([z.string(), z.number()]).optional(), url: z.string().optional(), path: z.string().optional(), timestamp: z.union([z.string(), z.number()]).optional() })).parse(JSON.parse(saved));
...
<section>
	...
	{#if loading}
		<div class="flex gap-3 overflow-hidden">
			{#each [1, 2, 3] as _}
				<div class="h-28 w-40 rounded-2xl bg-bg-elevated border border-fg/5 animate-pulse shrink-0"></div>
			{/each}
		</div>
	{:else if screenshots.length === 0}
		<div class="rounded-2xl bg-bg-elevated border border-fg/5 p-6 flex flex-col items-center justify-center text-center">
		...
		>
			{#each screenshots as shot (shot.id)}
				<div class="relative h-28 w-40 rounded-2xl overflow-hidden bg-bg-elevated border border-fg/5 hover:border-brand-400/30 transition-all shrink-0 group cursor-pointer">
					...
					{/if}
					<div class="absolute inset-0 bg-gradient-to-t from-bg-overlay/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
					{#if shot.timestamp}
						<span class="absolute bottom-1.5 left-2 text-[9px] font-bold text-fg/70 bg-bg-overlay/40 px-1.5 py-0.5 rounded-md backdrop-blur-sm">
							{shot.timestamp}
						</span>
					{/if}
				</div>
