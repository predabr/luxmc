<script lang="ts">
	...
	function getAccountStatus(acc: typeof account.value) {
		...
		return { type: "online", label: "Microsoft Online", dotColor: "bg-brand-400" };
	}

	const accountStatus = $derived(getAccountStatus(account.value));
</script>

<aside class="flex h-full min-h-screen w-[70px] shrink-0 flex-col items-center py-4 bg-bg/25 backdrop-blur-2xl border-r border-fg/[0.06] z-40 relative select-none">
	...
	>
		...
		<span 
			class="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-border {accountStatus.dotColor} shadow-sm"
			title={accountStatus.label}
		></span>

		<div class="pointer-events-none absolute left-[74px] top-1/2 -translate-y-1/2 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-150 z-[9999] whitespace-nowrap bg-bg-subtle text-fg text-xs font-bold px-3 py-1.5 rounded-xl border border-fg/10 shadow-2xl flex items-center gap-2">
	...
	<nav class="flex-1 w-full flex flex-col items-center gap-2">
		{#each items as item}
			...
			<div class="relative group w-full flex justify-center">
				{#if active}
					<span class="absolute left-0.5 top-1/2 -translate-y-1/2 w-1 h-6 bg-brand-400 rounded-r-full shadow-glow"></span>
				{/if}
				<a
					href={item.href}
					onclick={(e) => {
					...
					class="relative h-11 w-11 rounded-[16px] flex items-center justify-center transition-all duration-200 cursor-pointer {active ? 'nav-pill-active scale-[1.02]' : 'nav-pill-inactive active:scale-[0.98]'}"
				>
					<item.icon class="h-5 w-5 {active ? 'text-brand-400' : 'text-fg-muted group-hover:text-fg transition-colors'}" strokeWidth={active ? 2.2 : 1.8} />
				</a>
		...
		{#if profiles.list.length > 0}
			<div class="w-full flex flex-col items-center gap-2 overflow-y-auto overflow-x-hidden max-h-[28vh] custom-scrollbar px-1 py-0.5">
				{#each profiles.list as prof}
					...
					<div class="relative group w-full flex justify-center">
						{#if active}
							<span class="absolute left-0.5 top-1/2 -translate-y-1/2 w-1 h-5 bg-brand-400 rounded-r-full shadow-glow"></span>
						{/if}
						<a
						...
						<div class="pointer-events-none absolute left-[74px] top-1/2 -translate-y-1/2 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-150 z-[9999] whitespace-nowrap bg-bg-subtle text-fg text-xs font-bold px-3 py-2 rounded-xl border border-fg/10 shadow-2xl flex flex-col gap-0.5">
			...
			<div class="w-7 h-[1px] bg-fg/5 my-0.5"></div>
		{/if}
	...
	<div class="mt-auto w-full flex flex-col items-center gap-2 pt-2 border-t border-fg/5">
		<div class="relative group w-full flex justify-center">
			<button
				type="button"
				onclick={() => { void openPortal().catch(error => toast(String(error), "error")); }}
				class="relative h-11 w-11 rounded-[16px] flex items-center justify-center transition-all duration-200 cursor-pointer nav-pill-inactive active:scale-[0.98] group-hover:border-emerald-500/30"
				aria-label="Portal Web & Studio 3D"
			>
				<Globe class="h-5 w-5 text-fg-muted group-hover:text-emerald-400 transition-colors" strokeWidth={1.8} />
			</button>
			<div class="pointer-events-none absolute left-[74px] top-1/2 -translate-y-1/2 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-150 z-[9999] whitespace-nowrap bg-bg-subtle text-fg text-xs font-bold px-3 py-1.5 rounded-xl border border-fg/10 shadow-2xl flex items-center gap-1.5">
		...
		<div class="relative group w-full flex justify-center">
			<button
				...
				class="relative h-11 w-11 rounded-[16px] flex items-center justify-center transition-all duration-200 cursor-pointer nav-pill-inactive active:scale-[0.98] group-hover:border-fg/20"
				aria-label="Repositório GitHub"
			>
				<Github class="h-5 w-5 text-fg-muted group-hover:text-fg transition-colors" strokeWidth={1.8} />
			</button>
			<div class="pointer-events-none absolute left-[74px] top-1/2 -translate-y-1/2 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-150 z-[9999] whitespace-nowrap bg-bg-subtle text-fg text-xs font-bold px-3 py-1.5 rounded-xl border border-fg/10 shadow-2xl flex items-center gap-1.5">
		...
		<div class="relative group w-full flex justify-center">
			<a
				...
				class="relative h-11 w-11 rounded-[16px] flex items-center justify-center transition-all duration-200 cursor-pointer {settingsActive ? 'nav-pill-active scale-[1.02]' : 'nav-pill-inactive active:scale-[0.98]'}"
			>
