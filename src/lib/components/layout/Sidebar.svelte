...
<aside class="flex h-full min-h-screen w-[66px] shrink-0 flex-col items-center py-3 bg-bg/75 backdrop-blur-2xl border-r border-fg/10 z-40 relative select-none">
	
	<div class="relative group w-full flex justify-center mb-3">
		<button
			type="button"
			onclick={() => goto("/")}
			class="relative h-11 w-11 rounded-2xl flex items-center justify-center transition-all duration-200 cursor-pointer {isHomeActive ? 'bg-[#1bd96a] text-[#090a0f] shadow-lg shadow-[#1bd96a]/25 scale-[1.04]' : 'bg-fg/5 text-fg/60 hover:text-fg hover:bg-fg/10 active:scale-[0.98]'}"
			title="Início"
		>
	...
	<nav class="flex-1 w-full flex flex-col items-center gap-2">
		{#each items as item}
			...
			<div class="relative group w-full flex justify-center">
				...
				<a
					...
					class="relative h-11 w-11 rounded-2xl flex items-center justify-center transition-all duration-200 cursor-pointer {active ? 'bg-[#1bd96a]/15 text-[#1bd96a] border border-[#1bd96a]/30 scale-[1.02]' : 'bg-fg/[0.02] border border-transparent text-fg/50 hover:text-fg hover:bg-fg/[0.06] active:scale-[0.98]'}"
				>
				...
				{#each profiles.list as prof}
					...
					<div class="relative group w-full flex justify-center">
						...
						<a
							...
							class="relative h-10 w-10 rounded-xl overflow-hidden flex items-center justify-center transition-all duration-200 active:scale-[0.98] border cursor-pointer {active ? 'ring-2 ring-[#1bd96a] border-transparent shadow-lg scale-105' : 'border-fg/10 bg-fg/[0.03] hover:bg-fg/[0.08] hover:border-fg/30'}"
						>
		...
		<div class="relative group w-full flex justify-center mt-1">
			<a
				...
				class="h-10 w-10 rounded-xl bg-fg/[0.03] border border-fg/10 hover:border-[#1bd96a]/50 hover:bg-[#1bd96a]/10 hover:text-[#1bd96a] flex items-center justify-center text-fg/50 transition-all duration-200 active:scale-[0.98] shadow-sm cursor-pointer"
	...
	<div class="mt-auto w-full flex flex-col items-center gap-2 pt-2 border-t border-fg/10">
		<div class="relative group w-full flex justify-center">
			<button
				...
				class="relative h-10 w-10 rounded-xl flex items-center justify-center transition-all duration-200 cursor-pointer bg-fg/[0.02] text-fg/50 hover:text-[#1bd96a] hover:bg-fg/[0.06] active:scale-[0.98]"
		...
		<div class="relative group w-full flex justify-center">
			<a
				...
				class="relative h-10 w-10 rounded-xl flex items-center justify-center transition-all duration-200 cursor-pointer {settingsActive ? 'bg-[#1bd96a]/15 text-[#1bd96a] border border-[#1bd96a]/30' : 'bg-fg/[0.02] text-fg/50 hover:text-fg hover:bg-fg/[0.06] active:scale-[0.98]'}"
			>
		...
		>
			<div class="h-10 w-10 rounded-full overflow-hidden bg-fg/[0.04] border border-fg/10 group-hover:border-[#1bd96a] transition-all duration-200 shadow-sm flex items-center justify-center p-0.5">
				<img 
