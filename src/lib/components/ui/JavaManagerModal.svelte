...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
	<div class="w-full max-w-lg bg-bg-elevated border border-white/10 rounded-3xl shadow-2xl flex flex-col gap-0 overflow-hidden animate-slide-up">
		<div class="flex items-center justify-between px-6 py-4 border-b border-white/5">
			<div class="flex items-center gap-3">
				...
				<div>
					<h2 class="text-sm font-black text-white">Gerenciador de Java</h2>
					<p class="text-[11px] text-white/40">Detectar e instalar runtimes automaticamente</p>
				</div>
			</div>
			<div class="flex items-center gap-2">
				<button type="button" onclick={scan} class="p-2 rounded-xl text-white/40 hover:text-white hover:bg-white/5 transition-colors cursor-pointer" title="Verificar novamente">
					<RefreshCw class="w-4 h-4 {loading ? 'animate-spin' : ''}" />
				</button>
				<button type="button" onclick={onClose} class="p-2 rounded-xl text-white/40 hover:text-white hover:bg-white/5 transition-colors cursor-pointer">
					<X class="w-4 h-4" />
				</button>
			</div>
		</div>
		...
		<div class="p-6 flex flex-col gap-3">
			...
			{:else}
				{#each runtimes as runtime}
					...
					<div class="flex items-center justify-between gap-4 p-4 rounded-2xl border {runtime.installed ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-white/5 bg-bg-subtle'}">
						<div class="flex items-center gap-3 min-w-0">
							<div class="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 {runtime.installed ? 'bg-emerald-500/15 border border-emerald-500/25' : 'bg-white/5 border border-white/10'}">
								{#if runtime.installed}
									<CheckCircle class="w-4 h-4 text-emerald-400" />
								{:else}
									<AlertCircle class="w-4 h-4 text-white/30" />
								{/if}
							</div>
							<div class="min-w-0">
								<div class="flex items-center gap-2">
									<span class="text-sm font-bold {info?.color ?? 'text-white'}">{info?.label}</span>
									{#if runtime.isSystem}
								...
								<p class="text-[11px] text-white/40 truncate">{info?.use}</p>
								{#if runtime.versionString}
									<p class="text-[10px] text-white/30 font-mono truncate mt-0.5">{runtime.versionString}</p>
								{/if}
							</div>
						</div>
						...
						<div class="flex items-center gap-2 shrink-0">
							{#if installing === runtime.major}
								<div class="flex flex-col items-end gap-1.5 w-24">
									<span class="text-[10px] text-white/50">Instalando… {installProgress}%</span>
							...
							{:else if !runtime.isSystem}
								<button
									...
									class="p-2 rounded-xl text-white/30 hover:text-danger hover:bg-danger/10 border border-white/5 hover:border-danger/20 transition-all cursor-pointer"
		...
		<div class="px-6 pb-5 flex items-center justify-between">
			<a
				...
				class="flex items-center gap-1.5 text-[11px] text-white/30 hover:text-white/70 transition-colors"
			>
				<ExternalLink class="w-3 h-3" />
				Adoptium Temurin
			</a>
			<p class="text-[11px] text-white/30">Instalação automática via Mojang Launcher Meta</p>
		</div>
	</div>
</div>
{/if}
