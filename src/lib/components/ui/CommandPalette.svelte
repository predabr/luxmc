...
{#if open}
	...
	<div
		class="fixed inset-0 z-50 flex items-start justify-center pt-16 p-4 bg-black/75 backdrop-blur-md"
		onclick={() => (open = false)}
	...
	>
		...
		<div
			class="w-full max-w-xl bg-[#141518] border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-slide-up"
		...
		>
			<Command.Dialog {open} on:close={() => open = false}>
				<div class="flex items-center gap-3 px-4 border-b border-white/10">
					<Search class="w-4 h-4 text-white/40 shrink-0" />
					<Command.Input 
						value={query}
					...
					<div class="flex items-center gap-1.5 shrink-0">
						<kbd class="text-[10px] text-white/25 bg-white/5 border border-white/10 px-1.5 py-0.5 rounded-md font-mono">ESC</kbd>
						<button 
							type="button" 
				...
				<Command.List class="max-h-[360px] overflow-y-auto p-2 custom-scrollbar space-y-1">
					<Command.Empty class="py-8 text-center text-xs text-white/40">
						Nenhum resultado encontrado para "<span class="text-white/60">{query}</span>"
					</Command.Empty>

					<Command.Group heading="Navegação">
						<Command.Item value="inicio home jogar play" on:select={() => navigate("/")} class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs text-white/80 hover:text-white hover:bg-white/10 cursor-pointer transition-colors aria-selected:bg-white/10">
							<Play class="w-4 h-4 text-brand-500" />
							<span>Início</span>
						</Command.Item>
						<Command.Item value="instancias instances gerenciar" on:select={() => navigate("/instances")} class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs text-white/80 hover:text-white hover:bg-white/10 cursor-pointer transition-colors aria-selected:bg-white/10">
							<Layers class="w-4 h-4 text-emerald-400" />
							<span>Gerenciar Instâncias</span>
						</Command.Item>
						<Command.Item value="mods modpacks central conteudo" on:select={() => navigate("/mods")} class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs text-white/80 hover:text-white hover:bg-white/10 cursor-pointer transition-colors aria-selected:bg-white/10">
							<Package class="w-4 h-4 text-purple-400" />
							<span>Central de Conteúdo (Mods & Modpacks)</span>
						</Command.Item>
						<Command.Item value="skins capas skin capa personalizacao" on:select={() => navigate("/skins")} class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs text-white/80 hover:text-white hover:bg-white/10 cursor-pointer transition-colors aria-selected:bg-white/10">
						...
						<Command.Item value="configuracoes settings launcher tema" on:select={() => navigate("/settings")} class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs text-white/80 hover:text-white hover:bg-white/10 cursor-pointer transition-colors aria-selected:bg-white/10">
					...
					{#if filteredProfiles.length > 0}
						<Command.Group heading="Instâncias">
							{#each filteredProfiles as p}
								<Command.Item value={`instancia ${p.name} ${p.mcVersion} ${p.loader}`} on:select={() => { profiles.activeId = p.id; navigate(`/instances/${p.id}`); }} class="flex items-center justify-between px-3 py-2.5 rounded-xl text-xs text-white/80 hover:text-white hover:bg-white/10 cursor-pointer transition-colors aria-selected:bg-white/10">
					...
					<Command.Group heading="Ferramentas">
						<Command.Item value="java runtime jvm instalar gerenciar" on:select={openJavaManager} class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs text-white/80 hover:text-white hover:bg-white/10 cursor-pointer transition-colors aria-selected:bg-white/10">
							<Coffee class="w-4 h-4 text-orange-400" />
							<span>Gerenciador de Java</span>
						</Command.Item>
						<Command.Item value="tema cor aparencia visual" on:select={() => navigate("/settings")} class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs text-white/80 hover:text-white hover:bg-white/10 cursor-pointer transition-colors aria-selected:bg-white/10">
							<Palette class="w-4 h-4 text-violet-400" />
							<span>Tema & Aparência</span>
						</Command.Item>
						<Command.Item value="som audio silenciar efeitos" on:select={toggleSound} class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs text-white/80 hover:text-white hover:bg-white/10 cursor-pointer transition-colors aria-selected:bg-white/10">
				...
				<div class="px-3 py-2 border-t border-white/5 flex items-center justify-between">
					...
					<span class="text-[10px] text-white/20 font-mono">Ctrl+K</span>
				</div>
			</Command.Dialog>
		</div>
	</div>
