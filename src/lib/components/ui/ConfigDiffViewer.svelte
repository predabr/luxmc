...
<div class="flex flex-col gap-2 rounded-2xl border border-fg/10 bg-bg-subtle p-4 font-mono text-xs overflow-hidden">
	...
	<div class="max-h-72 overflow-y-auto custom-scrollbar flex flex-col">
		{#each diffs as part}
			<div 
				class="px-2 py-0.5 whitespace-pre-wrap transition-colors {part.added ? 'bg-emerald-500/20 text-emerald-300' : part.removed ? 'bg-rose-500/20 text-rose-300' : 'text-fg/80'}"
			>
