...
{#if isOpen}
	<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md select-none" in:fade={{ duration: 150 }}>
		<div class="w-full max-w-3xl rounded-3xl bg-[#141518] border border-white/15 p-6 shadow-2xl space-y-5 max-h-[90vh] flex flex-col" in:scale={{ start: 0.95, duration: 200 }}>
			<!-- Header -->
				...
				<div class="flex items-center gap-3">
					...
					<div>
						<div class="flex items-center gap-2">
							<h3 class="text-base font-black text-white">Gerenciador de Controles & Teclas</h3>
							{#if totalConflicts > 0}
						...
						<p class="text-xs text-white/50 mt-0.5">Edite atalhos e resolva teclas conflitantes sem precisar entrar no jogo</p>
					</div>
				</div>
			...
			<div class="flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
				<div class="flex bg-[#18191c] border border-white/10 rounded-full p-1 gap-1 overflow-x-auto w-full sm:w-auto">
					{#each categories as cat}
						<button
							type="button"
							class="px-3.5 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer whitespace-nowrap {activeCategory === cat ? 'bg-[#25262c] text-white shadow-sm border border-white/10' : 'text-white/40 hover:text-white'}"
				...
				<div class="relative w-full sm:w-60">
					...
					<input
						...
						class="w-full pl-9 pr-3 py-1.5 rounded-full bg-[#18191c] border border-white/10 text-xs text-white focus:outline-none focus:border-brand-500"
					/>
				</div>
			</div>
			...
			<div class="flex-1 overflow-y-auto pr-1 space-y-2 custom-scrollbar">
				{#if isLoading}
					<div class="p-12 text-center text-white/40 flex flex-col items-center justify-center gap-2">
				...
				{:else}
					{#each filteredKeybinds as k}
						<div class="p-3 rounded-2xl border transition-all flex items-center justify-between {k.isConflict ? 'bg-rose-500/10 border-rose-500/30' : 'bg-[#18191c] border-white/5 hover:border-white/15'}">
							<div class="min-w-0 pr-4">
								<div class="flex items-center gap-2">
									<span class="text-xs font-bold text-white truncate">{k.label}</span>
									<span class="text-[9px] font-mono text-white/40 bg-white/5 px-2 py-0.5 rounded">{k.category}</span>
									{#if k.isConflict}
								...
								<span class="text-[10px] text-white/40 font-mono block mt-0.5">{k.id}</span>
							</div>

							<button
								type="button"
								class="px-4 py-2 rounded-xl text-xs font-mono font-bold transition-all cursor-pointer min-w-[90px] text-center {listeningKeyId === k.id ? 'bg-brand-500 text-black animate-pulse shadow-lg' : k.isConflict ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40 hover:bg-rose-500/30' : 'bg-[#222328] hover:bg-[#2c2d35] text-white/90 border border-white/10'}"
			...
			<div class="flex items-center justify-between pt-3 border-t border-white/10 shrink-0">
				<span class="text-xs text-white/40">Clique em qualquer tecla para redefinir pelo teclado</span>
				...
				<div class="flex items-center gap-2.5">
					...
					<button
						type="button"
						class="px-7 py-2.5 rounded-2xl bg-brand-500 hover:bg-[#ebd08f] text-black font-black text-xs transition-all shadow-md cursor-pointer disabled:opacity-50"
						onclick={handleSave}
						disabled={isSaving}
					>
