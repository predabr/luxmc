	...
	<div class="space-y-5">
		...
		<div>
			...
			<div class="flex bg-[#0f1013] p-1 rounded-xl border border-white/5 gap-1">
				<button
					type="button"
					class="flex-1 py-1.5 px-2 rounded-lg text-xs font-semibold transition-all cursor-pointer flex items-center justify-center {selectedSource === 'all' ? 'bg-[#282935] text-white shadow-sm border border-white/10' : 'text-white/50 hover:text-white'}"
				...
				>
					<Flame class="w-3.5 h-3.5 text-[#f16436]" /> Curse
				</button>
			</div>
		</div>
